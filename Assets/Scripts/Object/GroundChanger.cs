...
public class GroundChanger : MonoBehaviour {
    ...
    public static IEnumerator ChangeGroundOnWalk(Transform playerObj, Transform groundT,Vector3 normal)
    {
        ...
        Debug.DrawRay(Vector3.zero, playerObj.up * 3, Color.green, 3);// 기존 y축 벡터
        Debug.DrawRay(Vector3.zero, normal * 3, Color.red,3);// 바뀔 y축 벡터
        Debug.DrawRay(Vector3.zero, outerProduct, Color.blue,3);// 외적

        while (fromToRate < 1)// && StateManager.currentGround == (groundT.gameObject))
        {
            Debug.DrawRay(Vector3.zero, playerObj.up, Color.black, 5);// 흔적
